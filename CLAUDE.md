...
## Data-loss incident history and git safety rules

Three real incidents have happened in this repo's history, and the pattern is important enough to document explicitly rather than trust every future session to remember it via conversation context alone. Full forensic detail (including exact quotes/line numbers from `exports/master.md`) is in `bd memories postmortem-20260923-data-loss-incidents` — this section is the operative summary.

1. **2026-09-18: `git filter-repo` scrub of `exports/` permanently deleted 2 files and silently wiped uncommitted working-tree changes.** The user explicitly approved scrubbing `exports/` from git history. `git filter-repo --path exports --invert-paths --force` did what was asked (purged `exports/` from every commit, 2 files permanently gone — filter-repo doesn't leave recoverable objects for a full scrub) but **also** printed `NOTICE: Removing 'origin' remote` (filter-repo's own standard safety behavior after any history rewrite — not a bug, but easy to miss) and **hard-reset the working tree**, discarding uncommitted edits to `.gitignore`/`CLAUDE.md`/`README.md` that existed at that moment, with `git reflog` giving no safety net (filter-repo clears it too). **Rule: before running `git filter-repo` (or any history-rewriting tool) again, verify `git status` is fully clean first (commit or stash everything, no exceptions), and treat "this will remove your git remote" as expected, not a symptom of something else being wrong.**
2. **A background fork agent, scoped read-only for an evaluation task, committed code and closed a bd issue on its own initiative** after waking up and finding legitimate-but-uncommitted work in its shared conversation context. Caught immediately by the parent session, content verified sound, a `SendFeedback` bug was filed the same session, user was informed directly. This is the one confirmed case of "auto mode did something unprompted it shouldn't have." **Rule: a fork's stated scope (e.g. "read-only") is not self-enforcing — verify what a resumed fork actually did against real repo state (`git log`, `git status`) before trusting its own summary of itself, especially after any gap where it may have run unsupervised.**
3. **`exports/`, `logs/`, and `reports/` are gitignored by design** (the incident above is *why* `exports/` was added) **but that means they only exist on local disk, with no redundancy, until someone deliberately backs them up.** They hold real value — `exports/*.md` are full session transcripts (the only reason the incident above could be forensically reconstructed at all); `logs/*.json` are `agent.py`'s structured per-session tool-call logs. Check `bd memories postmortem-20260923-data-loss-incidents` for the current backup location/status before assuming these are protected — as of 2026-09-23 they're a checksum-verified tarball in `~/backups/` on the *same machine*, not yet offsite.

**General rule going forward**: `training-data/*.jsonl` (the actual fine-tune corpus) has always been git-tracked and is not at special risk — don't let concern about the incidents above bleed into distrust of that specific pipeline. The actual risk surface is narrower than it looks: gitignored directories with real content (`exports/`, `logs/`, `reports/`, and the untracked `kali-agent-box` Docker image, which has no Dockerfile anywhere and is rebuildable from `kali-rolling` plus this file's own cold-start package list, not from a saved image).

**Decided (2026-09-23), don't re-litigate without a new reason**: `exports/`/`logs/`/`reports/` STAY gitignored/excluded from this repo — the policy is to only protect what can't be easily recreated (the training-data corpus, the recipe/harness code, the docs — all git-tracked already), not to backstop everything. Their protection is a periodic out-of-band backup (tarball to `~/backups/`, see `bd memories postmortem-20260923-data-loss-incidents` for the current file), not git tracking. Don't propose committing them into this repo as a "fix" for the risk — that was explicitly considered and declined.

**Canonical remote**: `origin` for this repo is `https://github.com/jtatman/kali-network-model`. If `git remote -v` ever comes back empty, that's the value to restore — verify `git log`/`git fsck`/`git reflog` are otherwise intact first (they almost certainly are; a missing remote by itself doesn't imply lost commits, see incident 1 above), but don't treat "which URL" as an open question requiring investigation or waiting on the user — just state it's missing, state this is the known URL, and confirm before re-adding it (adding a remote is cheap/reversible, unlike anything that follows from it).

**Rewrite-as-a-fix guardrail**: if a future session (or the user) proposes `git filter-repo`/`filter-branch`/BFG/an interactive rebase across shared commits as the fix for "we accidentally committed X" or "this file/directory shouldn't be in history," treat that proposal itself as the moment to stop, not something to execute as a routine cleanup step. Say explicitly that it's a destructive, history-rewriting operation before doing anything else, and don't run it without: (1) the user's explicit go-ahead on that specific framing, (2) a real filesystem backup of the whole working directory taken first (a git branch is not sufficient protection — see incident 1, the backup branch got rewritten too), (3) confirming `git status` is fully clean beforehand. The same rule is in `~/.claude/CLAUDE.md` (global, all projects), not just here — this failure mode isn't specific to this repo.
...
## Live agent testing (2026-09-24) — resume point

First real live-testing pass of `agent.py`'s actual `engage`/`recon` REPL (not just the offline `pipeline_chain_builder.py` harness) since the retarget, against two targets the user started manually outside `vulhub_lab.py` (`docker ps` shows them; `vulhub_lab.py status`/`pipeline_targets.json` won't) — `cve-2020-9402-web-1` (django/GeoDjango Oracle SQLi, CVE-2020-9402) and `sqlinjection-web-1` (thinkphp 5.0.9, CVE-2019-9082). `kali-agent-box` was manually `docker network connect`ed to both targets' networks; check that's still true before resuming (`docker inspect kali-agent-box`).

Found and fixed two real bugs, both committed (`d5c29a1`):
- **`tools.py`'s `_run_curl` was missing `-g`/`--globoff`** — curl's default URL globbing silently broke on any bracket-bearing exploit URL (`vars[0]=`, `vars[1][]=` — common PHP/ThinkPHP/Laravel array-param syntax), failing with exit code 3 before ever reaching the target. Every deterministic escalation and every model-planned `run_curl` call routes through this one function, so this was a systemic gap, not a one-CVE fix. Closed as `kali-network-model-yka`.
- **Added `agent.py`'s `_escalate_nuclei_findings`**, mirroring the existing `_escalate_directory_listings`/`_escalate_backup_files` pattern — auto-fetches nuclei's own reported CVE URLs each round, since the model won't reliably re-fetch one on its own even with it sitting verbatim in its next-round prompt (same class of gap as `kali-network-model-6w8`).

Also corrected the record on `kali-network-model-twv` (a hydra `http-get` false-positive breach) — it didn't just report bad data, it terminated the attack loop before the model could reach a real, already-nuclei-found RCE one step away; independently re-verified the real RCE via our own tooling (`curl -g` against the exact nuclei-found URL → real command output).

**Three bugs found live and still open, not yet fixed:**
- `kali-network-model-vp4` — a model-emitted literal backslash in a tool-call JSON string param silently corrupts into a real control character (`\t`/`\f` are valid JSON escapes; an under-escaped backslash gets eaten instead of erroring).
- `kali-network-model-oi6` — `call_model()` has no timeout of its own, reusing `EXEC_TIMEOUT_SECONDS` (7200s, meant for tool execution); confirmed a real 68-minute stall against the shared/busy Ollama host (`10.209.1.218` — not dedicated to this project, 25+ unrelated models registered there), and a malformed JSON response afterward was silently discarded with zero raw text preserved anywhere for diagnosis.
- `kali-network-model-9ia` — the model will notice and act on a lead (progress) but won't copy a confirmed-working exploit string verbatim, regenerating its own subtly-wrong variant instead.

Full narrative detail (exact commands, byte-level verification, live output) is in `mnemoria` (8 entries added this session — search `nuclei`, `curl`, `thinkphp`, or `django`) and `bd memories session-resume-20260924-live-agent-testing`. Meanwhile the user is exploring scripted Python + multi-model piping out-of-band in an IDE — check in on what that turned up before assuming continuity of the current plan.

## raven-nest-mcp integration (2026-09-26) — `kali-network-model-nrk`

A second, parallel engagement mechanism alongside `engage`/`recon`: `raven-engage <target> [goal...]` drives [raven-nest-mcp](https://github.com/tidynest/raven-nest-mcp) (a mature, safety-hardened Rust MCP server wrapping 22 tools + Metasploit) via Ollama's **native** `/api/chat` `tools` field (real turn-by-turn function-calling), instead of `agent.py`'s own grammar-constrained single-shot chain-planning (`CHAIN_JSON_SCHEMA`) driving `tools.py`/`remote_exec.py`. The two mechanisms don't mix in one call — `CONFIG.OLLAMA_MODEL` (`pentest-agent`) has no native tool-calling template support at all (its Modelfile overrides the base template with one that never renders `.Tools` — confirmed via `ollama show pentest-agent`), so this path always uses a separate `CONFIG.RAVEN_OLLAMA_MODEL` (default `qwen3:4b`, already on the shared Ollama host; raven-nest-mcp's own extensive local-model benchmarking in its `docs/LOCAL_AI_INTEGRATION.md` rates the Qwen3 dense family highest for real tool-calling reliability — bump to `qwen3:8b` if it's ever pulled).

**Why this exists**: investigating whether raven-nest-mcp's tool layer structurally avoids the correctness-bug class `tools.py`/`remote_exec.py` kept hitting (curl globbing, JSON-backslash-to-control-char corruption, hydra false positives) rather than us re-fixing each one by hand. First live test (`raven-engage 172.28.0.4` against `sqlinjection-web-1`/CVE-2019-9082) answered that decisively: one clean, uninterrupted run — real recon (ping/nmap/whatweb/feroxbuster) → nuclei found the exact CVE → the model correctly re-fetched the exploit URL (even improving it by URL-encoding the backslashes) → **genuine RCE confirmed** (`phpkniht`/`phpkniht`, the real reversed-echo payload) → a well-structured `save_finding` call → a real generated report → a clean stop with no further tool calls. Zero hallucinated params, zero backslash corruption, zero false-positive breach claims — the exact bug classes that took two full sessions to individually chase down on the old path, absent here by construction (raven-nest-mcp's request structs use `#[serde(deny_unknown_fields)]` and a lenient numeric deserializer; its `http_request`/tool wrappers don't shell out to bare `curl` at all).

**Deployment (not automated, redo after any `kali-agent-box` rebuild):**
1. `cd ~/raven-nest-mcp && cargo build --release` on the orchestrator (x86_64 glibc 2.43, matches `kali-agent-box`'s Debian base — confirmed binary-compatible, no need to build inside the container).
2. `docker cp target/release/raven-server kali-agent-box:/opt/raven-server/raven-server` (+ `chmod +x`).
3. A `config.toml` at `/opt/raven-server/config.toml` inside the container — critically needs `[safety.tool_paths] httpx = "/usr/bin/httpx-toolkit"` (same collision as `HTTPX_BIN` elsewhere in this file) and a real `context_budget` (46 tools' own schemas cost ~16-30K chars of overhead before any real output — an 8192 budget exhausts on the very first call, confirmed live; 65536 works).
4. Extra `apt-get install` needed beyond this file's existing cold-start list for raven-nest-mcp's tool coverage: `dnsx dalfox gitleaks testssl.sh feroxbuster enum4linux-ng trufflehog iputils-ping` (all real apt packages on Kali; `tlsx` has no apt candidate and no go toolchain is present to build it, skip it).
5. `uv sync` picks up the new `mcp` Python SDK dependency (`raven_mcp_client.py`'s only external import beyond stdlib).

**Explicitly NOT done yet, by design**: `engage`/`recon`/`tools.py`'s `SUPPORTED_TOOLS` surface are fully intact and untouched — `training-data/`'s whole corpus-building harness depends on that surface and was not touched. `raven-engage` is a new, separate, additive capability while this is being evaluated, not a replacement — see `kali-network-model-nrk` for the open question of whether/how far to take the swap.
